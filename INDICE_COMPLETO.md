# Índice Completo - Trilha de Carreira Data & IA

> Navegação rápida por todos os documentos do framework

---

## 📖 Documentação Principal

### Leitura Obrigatória
1. **[README.md](README.md)** - Documentação principal do framework
2. **[SUMARIO.md](SUMARIO.md)** - Guia de navegação e estrutura
3. **[NIVEIS_REFERENCIA.md](NIVEIS_REFERENCIA.md)** - Matriz de Scope/Reach/Impact

### Materiais de Apresentação
4. **[RESUMO_EXECUTIVO.md](RESUMO_EXECUTIVO.md)** - Resumo de 1 página para liderança
5. **[REVISAO_PRE_APRESENTACAO.md](REVISAO_PRE_APRESENTACAO.md)** - Análise completa do projeto
6. **[SLIDES_APRESENTACAO.md](SLIDES_APRESENTACAO.md)** - 22 slides executivos
7. **[CHECKLIST_FINAL.md](CHECKLIST_FINAL.md)** - Checklist de revisão

---

## 👥 Níveis de Carreira (9 arquivos)

### Individual Contributor (IC) Track

#### Níveis Iniciais
8. **[levels/SE_I_junior.md](levels/SE_I_junior.md)** - Software Engineer I (Junior)
   - 4 dimensões + Craft
   - Foco: Learning & Craft

9. **[levels/SE_II_pleno.md](levels/SE_II_pleno.md)** - Software Engineer II (Pleno)
   - 4 dimensões + Craft
   - Foco: Craft & Mentorship

10. **[levels/SE_III_senior.md](levels/SE_III_senior.md)** - Software Engineer III (Senior)
    - 4 dimensões + Craft
    - Foco: Project Leadership & Technical Excellence

#### Níveis Avançados
11. **[levels/Lead_engineer.md](levels/Lead_engineer.md)** - Lead Engineer (Senior Plus)
    - 4 dimensões + Craft
    - Foco: Technical Strategy & Project Leadership

12. **[levels/Staff_engineer.md](levels/Staff_engineer.md)** - Staff Engineer (Staff I)
    - 4 dimensões + Craft
    - Foco: Domain Expertise & Technical Strategy

13. **[levels/Staff_II_senior_staff.md](levels/Staff_II_senior_staff.md)** - Staff II (Senior Staff)
    - 4 dimensões + Craft
    - Foco: Organizational Strategy & Talent Multiplier

14. **[levels/Principal_engineer.md](levels/Principal_engineer.md)** - Principal Engineer
    - 4 dimensões + Craft
    - Foco: Company-Wide Strategy & Industry Leadership

### Management Track

15. **[levels/Tech_lead.md](levels/Tech_lead.md)** - Tech Lead (TL)
    - 4 dimensões + responsabilidades
    - Foco: Technical Leadership + Team Development
    - Hands-on: 50-70%

16. **[levels/Engineering_manager.md](levels/Engineering_manager.md)** - Engineering Manager (EM)
    - 4 dimensões + responsabilidades
    - Foco: People Development + Team Performance
    - Hands-on: <20%

---

## 🛠️ Trilhas Técnicas (3 arquivos)

### Por Especialização

17. **[tracks/data_engineering.md](tracks/data_engineering.md)** - Data Engineering
    - Pipelines, infraestrutura, arquitetura de dados
    - Stack: BigQuery, Fivetran, Airflow, dbt, SQL, Python
    - Skills por nível: SE I → Principal

18. **[tracks/analytics_engineering.md](tracks/analytics_engineering.md)** - Analytics Engineering
    - Modelagem dimensional, dbt, métricas, BI
    - Stack: dbt (core), SQL (expert), BigQuery, Looker
    - Skills por nível: SE I → Principal

19. **[tracks/cientista_de_dados.md](tracks/cientista_de_dados.md)** - Cientista de Dados / ML Engineer
    - ML, IA, GenAI/LLMs, experimentação
    - Stack: Python, PyTorch, LangChain, Vertex AI
    - Skills por nível: SE I → Principal

---

## 📈 Processo de Promoção (5 arquivos)

### Templates e Guias

20. **[promotion/template_promotion.md](promotion/template_promotion.md)** - Template Detalhado de Promoção
    - Estrutura completa baseada em 4 dimensões
    - Seções: Informações básicas, evidências, feedback, comparação
    - Uso: Documentar caso completo de promoção

21. **[promotion/calibration_guide.md](promotion/calibration_guide.md)** - Guia de Calibração
    - Para líderes (EMs, TLs, Head, Staff+)
    - Processo de calibração passo-a-passo
    - Critérios de decisão objetivos
    - Armadilhas comuns e como evitar

22. **[promotion/examples.md](promotion/examples.md)** - Exemplos de Evidências
    - Exemplos concretos por nível (SE III → Principal)
    - Projetos de impacto típicos
    - Mentoria e influência

23. **[promotion/rubric.md](promotion/rubric.md)** - Rubrica de Avaliação
    - Dimensões principais de avaliação
    - Progressão esperada por nível
    - Uso: Referência rápida em calibrações

24. **[promotion/promotion_packet_template.md](promotion/promotion_packet_template.md)** - Template Alternativo
    - Versão mais simples e concisa
    - Foco em projetos de impacto e métricas
    - Uso: Para promoções mais diretas

---

## 💡 Competências Transversais (4 arquivos)

### Detalhamento de Competências

25. **[competencies/tech_excellence.md](competencies/tech_excellence.md)** - Excelência Técnica ⭐
    - 6 dimensões: Arquitetura, Qualidade, Tecnologia, Operação, Performance, Segurança
    - Evolução por nível (SE I → Principal)
    - Métricas de sucesso

26. **[competencies/execution.md](competencies/execution.md)** - Execução
    - *Status: Básico (pode ser expandido)*

27. **[competencies/communication.md](competencies/communication.md)** - Comunicação
    - *Status: Básico (pode ser expandido)*

28. **[competencies/leadership.md](competencies/leadership.md)** - Liderança
    - *Status: Básico (pode ser expandido)*

---

## 🌈 Cultura (1 arquivo)

### Integração com Valores Gupy

29. **[culture/mapping_to_gupy_culture.md](culture/mapping_to_gupy_culture.md)** - Mapeamento Cultural ⭐
    - 5 valores Gupy mapeados por nível
    - Como cada valor se manifesta (SE I → Principal)
    - Conexão com 4 dimensões do framework
    - Exemplos práticos

---

## 🔍 Guias Rápidos por Persona

### Para Individual Contributors (ICs)

**Quero entender meu nível atual:**
1. Leia [README.md](README.md) - Visão geral
2. Leia seu nível em [levels/](levels/)
3. Revise sua trilha em [tracks/](tracks/)

**Quero me preparar para promoção:**
1. Compare seu nível com o próximo em [levels/](levels/)
2. Use [NIVEIS_REFERENCIA.md](NIVEIS_REFERENCIA.md) para comparação
3. Preencha [promotion/template_promotion.md](promotion/template_promotion.md)
4. Converse com seu gestor

### Para Gestores (EMs, Tech Leads)

**Quero fazer 1:1 de carreira:**
1. Use 4 dimensões como framework
2. Referencie arquivos de [levels/](levels/)
3. Compare com [NIVEIS_REFERENCIA.md](NIVEIS_REFERENCIA.md)

**Quero propor uma promoção:**
1. Preencha [promotion/template_promotion.md](promotion/template_promotion.md)
2. Leia [promotion/calibration_guide.md](promotion/calibration_guide.md)
3. Prepare-se para calibração

**Quero calibrar promoções:**
1. Siga [promotion/calibration_guide.md](promotion/calibration_guide.md)
2. Use [promotion/rubric.md](promotion/rubric.md) como referência
3. Veja [promotion/examples.md](promotion/examples.md) para exemplos

### Para Líderes (Head, Staff+, Principal)

**Quero apresentar o framework:**
1. Leia [RESUMO_EXECUTIVO.md](RESUMO_EXECUTIVO.md)
2. Use [SLIDES_APRESENTACAO.md](SLIDES_APRESENTACAO.md)
3. Consulte [REVISAO_PRE_APRESENTACAO.md](REVISAO_PRE_APRESENTACAO.md)

**Quero governar e manter o framework:**
1. Veja seção Governança em [README.md](README.md)
2. Use [CHECKLIST_FINAL.md](CHECKLIST_FINAL.md) para revisões
3. Implemente ciclo de revisão (trimestral/anual)

---

## 📑 Documentos por Finalidade

### Onboarding / Entendimento Inicial
- [README.md](README.md)
- [SUMARIO.md](SUMARIO.md)
- Seu nível em [levels/](levels/)
- Sua trilha em [tracks/](tracks/)

### Desenvolvimento de Carreira
- [NIVEIS_REFERENCIA.md](NIVEIS_REFERENCIA.md)
- Nível atual e próximo em [levels/](levels/)
- Trilha técnica em [tracks/](tracks/)
- [competencies/tech_excellence.md](competencies/tech_excellence.md)

### Processo de Promoção
- [promotion/template_promotion.md](promotion/template_promotion.md)
- [promotion/calibration_guide.md](promotion/calibration_guide.md)
- [promotion/examples.md](promotion/examples.md)
- [promotion/rubric.md](promotion/rubric.md)

### Liderança e Gestão
- [RESUMO_EXECUTIVO.md](RESUMO_EXECUTIVO.md)
- [SLIDES_APRESENTACAO.md](SLIDES_APRESENTACAO.md)
- [REVISAO_PRE_APRESENTACAO.md](REVISAO_PRE_APRESENTACAO.md)
- [promotion/calibration_guide.md](promotion/calibration_guide.md)

### Cultura
- [culture/mapping_to_gupy_culture.md](culture/mapping_to_gupy_culture.md)
- Seções "Conexão com Cultura Gupy" em cada arquivo de [levels/](levels/)

---

## 📊 Estatísticas do Framework

**Total de documentos:** 31 arquivos markdown  
**Total de páginas:** ~300 páginas (estimado)  
**Níveis cobertos:** 9 (SE I → Principal + TL + EM)  
**Trilhas técnicas:** 3 completas  
**Dimensões de avaliação:** 4 + Craft  
**Status:** ✅ Pronto para implementação

---

## 🔗 Links Úteis

### Externos
- [Repositório Dropbox Original](https://github.com/dropbox/dbx-career-framework)
- [Artigo Dropbox sobre o Framework](https://dropbox.tech/infrastructure/sharing-our-engineering-career-framework-with-the-world)

### Internos (Gupy)
- Culture Code Gupy (link interno da empresa)
- Portal de RH (se aplicável)
- Canal Slack #carreira-data-ia (criar após lançamento)

---

## 💡 Dica de Navegação

**Este repositório é grande!** Para não se perder:

1. **Comece pelo README.md** - Visão geral
2. **Use o SUMARIO.md** - Mapa do framework
3. **Consulte este INDICE_COMPLETO.md** - Quando precisar encontrar algo específico

**Atalhos úteis:**
- Níveis: `levels/`
- Trilhas técnicas: `tracks/`
- Promoção: `promotion/`
- Cultura: `culture/`

---

**Última atualização:** Novembro 2024  
**Mantido por:** Liderança de Data & IA Engineering

